...
const rows = shallowRef<sys_companies[]>([]);
...
const refreshData = async () => {
  try {
    ...
    const resultSet = await $fetch('/api/security/companies', {
      method: 'POST',
      body: {
...
const selectRow = (row: sys_companies) => emits('row-click', row);
...
<template>
  <div>  
    <UTable
      ...
      :loading="isLoading"
      @select="selectRow">
      <template #id-data="{row} : {row: sys_companies}">
        <UAvatar
          v-if="row.avatar_url"
          :src="row.avatar_url"
          size="sm"
