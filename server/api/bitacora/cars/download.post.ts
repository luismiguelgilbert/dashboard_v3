...
import { bitacora_cars_query_download_schema } from '@/types/bitacora_cars';

export default defineEventHandler( async (event) => {
  try{
    await hasPermission(event, PermissionsList.BITACORA_CARS_EXPORT);
    const { data: payload, error } = await readValidatedBody(event, bitacora_cars_query_download_schema.safeParse);
    if (error) {
      throw createError({
        statusCode: 500,
