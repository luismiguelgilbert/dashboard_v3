...
const rows = shallowRef<bitacora_cars[]>([]);
...
const refreshData = async () => {
  try {
    ...
    const resultSet = await $fetch('/api/bitacora/cars', {
      method: 'POST',
      body: {
...
const selectRow = (row: bitacora_cars) => emits('row-click', row);
...
<template>
  <div>  
    <UTable
      ...
      :loading="isLoading"
      @select="selectRow">
      <template #id-data="{row} : {row: bitacora_cars}">
        <UAvatar
          v-if="row.avatar_url"
          :src="row.avatar_url"
          size="sm"
      ...
      <template #name_es_short-data="{row} : {row: bitacora_cars}">
      ...
      <template #name_es-data="{row} : {row: bitacora_cars}">
        <span class="sm:flex text-base text-gray-500 dark:text-gray-400 truncate text-ellipsis">
          {{ row.name_es }}
        </span>
      </template>
      <template #is_active-data="{row} : {row: bitacora_cars}">
        <UBadge
          variant="subtle"
