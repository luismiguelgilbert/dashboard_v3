    ...
    if (formModel.value === 'edit') {
      if (useRoute().query.id) {
        ...
        const results = await Promise.all([
          $fetch(`/api/bitacora/cars/:${userCompany.value?.id}/:${useRoute().query.id}`),
        ]);
...
const validateAndSave = async () => {
  try {
    ...
    isSaveSuccess.value = false;
    hasError.value = false;
    const carData = await $fetch(`/api/bitacora/cars/:${userCompany.value?.id}/:${useRoute().query.id}`, {
    ...
    if (selectedRowData.value?.avatar_url && selectedRowDataAvatarHelper.value ) {
      ...
      await $fetch(`/api/bitacora/cars/:${userCompany.value?.id}/:${carData.id}/avatar`, {
        method: 'PATCH',
        body: body,
      });
    }
        ...
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">
              {{ formModel === 'create' ? 'Crear' : 'Editar' }} vehículo:
            </h3>
