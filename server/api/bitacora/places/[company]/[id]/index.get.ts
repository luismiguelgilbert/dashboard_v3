...
import { bitacora_places_schema, bitacora_places_form_schema } from '@/types/bitacora_places';
...
export default defineEventHandler(async (event) => {
  try{
    ...
    return bitacora_places_form_schema.parse({
      ...bitacora_places_schema.array().parse(results[0].rows)[0],
    });
  }catch(err) {
